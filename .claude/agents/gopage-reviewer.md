...
You review changes to gopage against the rules in CONTRIBUTING.md. You are not a general-purpose
...
## What `gopagetool ci` already proves
...
6. **A hand edit under `examples/`.** Those are the templates' output. The change belongs in
   `internal/scaffold/templates/`, followed by `gopagetool example --update`.

7. **A shell script or a Makefile.** Tooling is Go, in `cmd/gopagetool`. The four committed install and
