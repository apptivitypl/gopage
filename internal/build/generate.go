...
func app() []byte {
	...
	b.WriteString("func Options() rill.Options {\n")
	b.WriteString("\treturn rill.Options{\n")
...
func provider(entry generated) []byte {
	...
	if entry.Loader {
		b.WriteString("func Provider(request *http.Request, params rill.Params) (rill.Accessible, error) {\n")
		fmt.Fprintf(&b, "\tprops, err := Load(%s)\n", loaderArgs(entry))
		b.WriteString("\tif err != nil {\n\t\treturn nil, err\n\t}\n")
		b.WriteString("\treturn props, nil\n}\n")
	}
	if entry.Meta {
		b.WriteString("\nfunc MetaProvider(request *http.Request, params rill.Params) (rill.Meta, error) {\n")
		b.WriteString("\tctx := rill.NewCtx(request, params)\n")
		fmt.Fprintf(&b, "\tprops, err := Load(%s)\n", metaLoaderArgs(entry))
		b.WriteString("\tif err != nil {\n\t\treturn rill.Meta{}, nil\n\t}\n")
		b.WriteString("\treturn Meta(ctx, props), nil\n}\n")
	}
	for _, name := range entry.Deferred {
		fmt.Fprintf(&b, "\nfunc %sProvider(request *http.Request, params rill.Params) (rill.Accessible, error) {\n", name)
		fmt.Fprintf(&b, "\tvalue, err := %s(rill.NewCtx(request, params))\n", name)
		b.WriteString("\tif err != nil {\n\t\treturn nil, err\n\t}\n")
		fmt.Fprintf(&b, "\treturn %s%s{value: value}, nil\n}\n", codegen.DeferredPrefix, name)
	}
	if entry.Submit {
		b.WriteString("\nfunc SubmitProvider(request *http.Request, params rill.Params) (rill.Action, rill.FormResult, error) {\n")
		fmt.Fprintf(&b, "\tvar submitted %s\n", entry.Form)
		b.WriteString("\tresult, err := rill.DecodeForm(request, &submitted)\n")
		b.WriteString("\tif err != nil {\n\t\treturn nil, result, err\n\t}\n")
		b.WriteString("\tif !result.Valid() {\n\t\treturn nil, result, nil\n\t}\n")
		b.WriteString("\taction, err := Submit(rill.NewCtx(request, params), params, submitted)\n")
...
func adapter(pkg string, handler compile.Handler, module string) []byte {
	...
	b.WriteString("\treturn rill.API(map[string]rill.APIHandler{\n")
	for _, method := range handler.Methods {
		fmt.Fprintf(&b, "\t\t%q: func(request *http.Request) (rill.Response, error) {\n", method)
		b.WriteString("\t\t\tbound := rill.ParamsFrom(request, params)\n")
		fmt.Fprintf(&b, "\t\t\treturn route.%s(rill.NewCtx(request, bound), bound)\n", method)
		b.WriteString("\t\t},\n")
	}
	b.WriteString("\t})\n}\n")
	return []byte(b.String())
...
func loaderArgs(entry generated) string {
	if entry.Params {
		return "rill.NewCtx(request, params), params"
	}
	return "rill.NewCtx(request, params)"
}
...
func registry(module string, packages, handlers []generated) []byte {
	...
	b.WriteString(")\n\n")
	b.WriteString("func Props() map[string]rill.PropsProvider {\n")
	b.WriteString("\treturn map[string]rill.PropsProvider{\n")
	...
	b.WriteString("\t}\n}\n\n")
	b.WriteString("func Meta() map[string]rill.MetaProvider {\n")
	b.WriteString("\treturn map[string]rill.MetaProvider{\n")
	for _, pkg := range packages {
		if pkg.Meta {
	...
	b.WriteString("\t}\n}\n\n")
	b.WriteString("func Submit() map[string]rill.SubmitProvider {\n")
	b.WriteString("\treturn map[string]rill.SubmitProvider{\n")
	for _, pkg := range packages {
		if pkg.Submit {
	...
	b.WriteString("\t}\n}\n\n")
	b.WriteString("func Deferred() map[string]rill.DeferredProvider {\n")
	b.WriteString("\treturn map[string]rill.DeferredProvider{\n")
	for _, pkg := range packages {
