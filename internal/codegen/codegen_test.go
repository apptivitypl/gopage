...
func build(t *testing.T, code string) *schema.Schema {
	t.Helper()
	var bag diag.Bag
	parsed := schema.Parse([]schema.Source{{File: "page.gopage", Code: code}}, &bag)
	if bag.HasErrors() {
...
type Props struct {
	...
	for _, want := range []string{
		`gopage.String(string(v.Title))`,
		`gopage.Int(int64(v.Count))`,
		`gopage.Float(float64(v.Ratio))`,
		`gopage.Bool(bool(v.Enabled))`,
	} {
...
func TestNestedStructDelegates(t *testing.T) {
	...
	if !strings.Contains(source, "gopage.Object(v.Listing)") {
...
type Card struct{ Title string }`)})
	for _, want := range []string{
		"gopage.Strings(v.Tags)",
		"gopage.Ints[int](v.Counts)",
		"gopage.Floats[float64](v.Ratios)",
		"gopage.Bools(v.Flags)",
		"gopage.Objects[Card](v.Cards)",
	} {
		if !strings.Contains(source, want) {
			t.Errorf("generated code is missing %s:\n%s", want, source)
		}
...
func TestOptionalScalar(t *testing.T) {
	...
	if !strings.Contains(source, "gopage.String(string((*v.Badge)))") {
...
func TestUnknownStructYieldsNothing(t *testing.T) {
	var bag diag.Bag
	parsed := schema.Parse([]schema.Source{{File: "p.gopage", Code: "type Props struct{ C Card }"}}, &bag)
	source := render(t, File{Schema: parsed})
	mustParse(t, source)
	if !strings.Contains(source, "return gopage.Nil(), false") {
...
func TestFrontmatterIsCopiedWithALineDirective(t *testing.T) {
	source := render(t, File{
		SourceFile: "app/listings/[id]/page.gopage",
		SourceLine: 2,
		Source:     "func Load() error { return nil }\n",
	})
	if !strings.Contains(source, "//line app/listings/[id]/page.gopage:2") {
...
func TestSourceWithoutTrailingNewlineIsClosed(t *testing.T) {
	source := render(t, File{SourceFile: "p.gopage", SourceLine: 1, Source: "var X = 1"})
	mustParse(t, source)
}
...
func TestImportsAreSortedAndDeduplicated(t *testing.T) {
	...
	if !strings.Contains(source, `"`+GopageImport+`"`) {
		t.Errorf("the gopage import is missing:\n%s", source)
	}
}

func TestPackageClause(t *testing.T) {
	source := render(t, File{Package: "listings_id"})
	if !strings.HasPrefix(source, "// Code generated by gopage. DO NOT EDIT.\n\npackage listings_id") {
		t.Errorf("package clause is wrong:\n%s", source)
	}
}

func TestBrokenSourceIsReportedWithTheText(t *testing.T) {
	out, err := Render(File{Package: "page", SourceFile: "p.gopage", SourceLine: 1, Source: "func ("})
	if err == nil {
...
type Props struct {
	...
	mustParse(t, source)
	for _, want := range []string{
		"func (v Status) gopageCase() string",
		`case StatusActive:`,
		`return "StatusActive"`,
		"gopage.String(v.State.gopageCase())",
		"gopage.Cases[Status](v.States)",
	} {
		if !strings.Contains(source, want) {
			t.Errorf("generated code is missing %s:\n%s", want, source)
		}
...
func TestEnumWithoutConstantsGetsNoMethod(t *testing.T) {
	...
	if strings.Contains(source, "gopageCase") {
...
`,
	...
	if !strings.Contains(string(source), "//line app/page.gopage:3\n\ntype Props struct {") {
...
func TestAnEmptyFrontmatterEmitsNoDirective(t *testing.T) {
	source, err := Render(File{Package: "page", SourceFile: "app/page.gopage", SourceLine: 2, Source: "   \n"})
	if err != nil {
		t.Fatalf("Render: %v", err)
	}
...
func TestComputedFieldsAreCalled(t *testing.T) {
	...
	text := string(source)
	if !strings.Contains(text, "gopage.Int(int64(v.PricePerM2()))") {
		t.Errorf("generated = %q, want the method called", text)
	}
	if !strings.Contains(text, "gopage.Int(int64(v.Price))") {
...
func Count(ctx *gopage.Ctx) (int, error) {
	return 0, nil
}

func One(ctx *gopage.Ctx) (Review, error) {
	return Review{}, nil
}
`
	var bag diag.Bag
	model := schema.Parse([]schema.Source{{File: "app/page.gopage", Code: source}}, &bag)
	out, err := Render(File{Package: "index", SourceFile: "app/page.gopage", SourceLine: 2, Source: source, Schema: model})
	if err != nil {
		t.Fatalf("Render: %v", err)
	}
	code := string(out)
	for _, want := range []string{
		...
		"gopage.Seq(gopage.Objects[Review](d.value))",
		"type deferredCount struct",
		"gopage.Int(int64(d.value))",
...
func Text(ctx *gopage.Ctx) (string, error) {
	return "", nil
}

func Ratio(ctx *gopage.Ctx) (float64, error) {
	return 0, nil
}

func Ready(ctx *gopage.Ctx) (bool, error) {
	return false, nil
}

func Names(ctx *gopage.Ctx) ([]string, error) {
	return nil, nil
}
...
func Maybe(ctx *gopage.Ctx) (*Row, error) {
	return nil, nil
}
`
	var bag diag.Bag
	...
	for _, want := range []string{
		...
		"gopage.Strings(d.value)",
		"gopage.Objects[Row](d.value)",
	} {
		if !strings.Contains(code, want) {
...
func Badge(ctx *gopage.Ctx) (Tone, error) {
	return ToneOk, nil
}

func Tones(ctx *gopage.Ctx) ([]Tone, error) {
	return nil, nil
}
`
	var bag diag.Bag
...
func Client(ctx *gopage.Ctx) (http.Client, error) {
	return http.Client{}, nil
}

func Blobs(ctx *gopage.Ctx) ([]http.Client, error) {
	return nil, nil
}
`
	var bag diag.Bag
	...
	if !strings.Contains(string(out), "gopage.Nil()") {
...
func Total(ctx *gopage.Ctx) (Count, error) {
	return 0, nil
}

func Totals(ctx *gopage.Ctx) ([]Count, error) {
	return nil, nil
}
`
	var bag diag.Bag
