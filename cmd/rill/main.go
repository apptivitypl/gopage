...
func run(args []string) error {
	if len(args) == 0 {
		return fail("rill needs a command").
			showing(usage()).
			try("rill new my-site --module example.com/my-site", "rill dev")
	}
	switch args[0] {
	case "new":
...
func usage() string {
	...
	fmt.Fprintf(&b, "  %s %s\n\n", out.paint(bold, "rill"), out.paint(dim, "a web framework in go"))
	fmt.Fprintf(&b, "  %s\n    rill <command> [flags]\n\n", out.paint(dim, "usage"))
...
func newProject(args []string) error {
	...
	if len(positional) != 1 {
		return fail("rill new takes one directory, and was given %d", len(positional)).
			...
			try("rill new my-site --module example.com/my-site")
	}
	...
	if err := scaffold.Create(cfg); err != nil {
		if errors.Is(err, scaffold.ErrNotEmpty) {
			return fail("%s already holds files", cfg.Dir).
				because("rill new writes a whole project tree and will not merge into a directory that is already in use, so nothing here is overwritten.").
				try("rill new "+cfg.Dir+"-2 --module "+moduleOrPlaceholder(cfg), "rm -rf "+cfg.Dir+" && rill new "+cfg.Dir+" --module "+moduleOrPlaceholder(cfg))
		}
		return err
	}
...
func nearest(typed string) []string {
	...
	for _, known := range commands() {
		if within(typed, known.name) {
			close = append(close, "rill "+known.name)
		}
	}
	if len(close) == 0 {
		return []string{"rill new my-site --module example.com/my-site", "rill dev"}
	}
	return close
}
