...
import (
	...
	"github.com/apptivitypl/gopage/internal/tool/smoke"
)
...
func smokeCmd(args []string) error {
	...
	dir, err := os.MkdirTemp("", "gopage-smoke-")
	if err != nil {
		return err
	}
	if !keep {
...
func scaffoldProject(root, project, template, locales string) error {
	steps := []struct {
		...
		{root, "go", []string{"run", "./cmd/gopage", "new", project,
			"--module", smokeModule, "--template", template, "--gopage-path", root,
...
func buildProject(root, project, target string) error {
	cmd := exec.Command("go", "run", "./cmd/gopage", "build", "--dir", project, "--target", target)
	cmd.Dir = root
	cmd.Stdout = os.Stderr
	cmd.Stderr = os.Stderr
	return cmd.Run()
...
func probeDev(root, project string) error {
	...
	binary := filepath.Join(filepath.Dir(project), "gopage")
	compile := exec.Command("go", "build", "-o", binary, "./cmd/gopage")
	compile.Dir = root
	compile.Stderr = os.Stderr
	if err := compile.Run(); err != nil {
		return fmt.Errorf("building gopage: %w", err)
	}
	...
	fetch := smokeFetcher()
	if err := smoke.WaitReady(fetch, base, readyAttempts, readyPause); err != nil {
		return fmt.Errorf("gopage dev: %w", err)
	}
	if err := smoke.RunDev(fetch, base); err != nil {
		return fmt.Errorf("gopage dev: %w", err)
	}

	page := filepath.Join(project, "app", "page.gopage")
	...
	if err := waitFor(fetch, base, func() error { return smoke.RunDev(fetch, base) }); err != nil {
		return fmt.Errorf("gopage dev did not recover: %w", err)
	}
	fmt.Println("smoke: ok — gopage dev served the application, rebuilt it and survived a broken template")
	return nil
}
