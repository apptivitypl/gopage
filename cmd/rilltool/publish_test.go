...
func TestWaitingGivesUpWithSomethingActionable(t *testing.T) {
	...
	err := job.settle([]string{"@apptivitypl/rill-linux-x64"})
	if err == nil {
		t.Fatal("settle must give up rather than wait forever")
	}
	for _, want := range []string{"rill-linux-x64", "run this again"} {
...
func TestWaitingStopsAsSoonAsEverythingIsThere(t *testing.T) {
	...
	if err := job.settle([]string{"@apptivitypl/rill-linux-x64"}); err != nil {
