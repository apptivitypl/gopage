...
This is what `gopage new my-site --template blog` writes: posts written in markdown, a page for each
...
<p>
  <a href="https://codesandbox.io/p/devbox/github/apptivitypl/gopage/tree/main/examples/blog"><img alt="open in codesandbox" src="https://assets.codesandbox.io/github/button-edit-lime.svg" height="30"></a>
</p>
...
| file | what it shows |
| --- | --- |
| `app/page.gopage` | the index; frontmatter is Go, and `Load` returns what the markup renders |
| `app/posts/[slug]/page.gopage` | a dynamic route; `params["slug"]` picks the post, and the loader sets a cache tag |
| `app/feed.xml/route.go` | a route that answers with something other than a page |
| `components/PostCard/` | a component big enough to want its own directory: `props.go` beside `template.gopage` |
| `content/` | markdown, and the Go that reads it |
| `gopage.jsonc` | plain css here, not Tailwind: the engine is a setting, not a rewrite |
