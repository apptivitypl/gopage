...
func demo(t *testing.T) []byte {
	...
	return build(t, fstest.MapFS{
		"app/layout.rill":             {Data: []byte("<main>{% outlet %}</main>")},
		"app/page.rill":               {Data: []byte("<h1>home</h1>")},
		"app/listings/[id]/page.rill": {Data: []byte("<p>{{ ID }}</p>")},
	})
}
...
func TestTheSidecarDecidesTheEarlyHints(t *testing.T) {
	bundles := fstest.MapFS{
		...
		"bundles/rill.preload": &fstest.MapFile{Data: []byte(
			"link </assets/rill.client.ABC.js>; rel=modulepreload\nlink </fonts/mono.woff2>; rel=preload; as=font; crossorigin\nisland Stars island.R.js\n")},
	}
	...
	if got := recorder.Header().Get("RILL-Assets"); !strings.Contains(got, "mono.woff2") || strings.Contains(got, "island.R.js") {
